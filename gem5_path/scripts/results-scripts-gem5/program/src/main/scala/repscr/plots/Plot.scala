...
abstract class Plot {
  ...
  def getValidTotalYValues: Iterable[Double] = getYValues filter { x => !x.isInfinity && !x.isNaN } map {
    _.value
  }
  def getYValuesRangeSize: Double = if (getValidTotalYValues.isEmpty) 0 else abs(getValidTotalYValues.max - getValidTotalYValues.min) match {
    case x if x.isInfinite => Double.MaxValue
    case x => x
  }

  var _yRangeMin: Option[Double] = None
  def yRangeMin_=(v: Double): Unit = {
    require(!v.isInfinite); _yRangeMin = Some(v)
  }
  def yRangeMin = _yRangeMin getOrElse {
    if (getValidTotalYValues.isEmpty) 0 else getValidTotalYValues.min - 0.05 * (getYValuesRangeSize max 0.000001)
  } ensuring { v => !v.isInfinite }

  var _yRangeMax: Option[Double] = None
  def yRangeMax_=(v: Double): Unit = {
    require(!v.isInfinite); _yRangeMax = Some(v)
  }
  def yRangeMax = _yRangeMax getOrElse {
    if (getValidTotalYValues.isEmpty) 0 + 0.05 * 0.000001 else (getValidTotalYValues.max max yRangeMin) + 0.05 * (getYValuesRangeSize max 0.000001)
  } ensuring { v => !v.isInfinite && v > yRangeMin }

  var _yGridInterval: Option[Double] = None
  def yGridInterval_=(v: Double): Unit = {
    _yGridInterval = Some(v)
  }
  ...
  protected def printPyChartCommonCode(): Unit = {
    ...
    print(
      s"""|my_fill_style = ${plotStyle.pyFillStyle}
          |
          |my_fill_style_len = len(my_fill_style)
          |""".stripMargin)

    print(
      s"""|my_line_style = ${plotStyle.pyLineStyle}
          |
          |my_line_style_len = len(my_line_style)
          |""".stripMargin)

    print(
      s"""|Infinity = ${Double.MaxValue}
          |NaN = 0
          |""".stripMargin)
  }
...
object Plot {
  ...
  object Style {
    case object Colors1 extends Style {
      def pyLineStyle =
        """[line_style.T(width = 1.5, color = my_fill_style[0 % my_fill_style_len].bgcolor, dash = None),
    ...
    case object Colors2 extends Style {
      ...
      def pyFillStyle =
        """[fill_style.Plain(bgcolor=color.lightblue),
    ...
    case object BW1 extends Style {
      ...
      def pyFillStyle =
        """[fill_style.Plain(bgcolor=color.gray10),
...
abstract class BarPlotCommon extends Plot {
  ...
  protected def printPyChartImprovementCode(): Unit = {
    ...
    print(
      """|class nodupslegend(pychart.legend.T):
  ...
  def printCode() = {
    ...
    if (seriesLength > 0) {
      ...
      for ((s, idx) <- series.zipWithIndex) {
        ...
        println(
          s"""|ar.add_plot(bar_plot.T(data = serie_data,
      ...
      print(
        s"""|tb = text_box.T(loc = (0,0), text="/hCEmpty")
            |tb.draw()
            |""".stripMargin)
    }
  }
  ...
  def printCode() = {
    ...
    if (series.nonEmpty && getCategories.nonEmpty) {
      ...
      if (seriesLegend) {
        ...
        print(
          s"""|legend_series_loc = (mm_to_pt($legendOffsetX), area_size[1] + mm_to_pt($legendOffsetY) - ${fontSize * (seriesLegendRows + 0.5)})
...
class LinePlot extends Plot {
  ...
  def xRangeMin_=(v: Double): Unit = {
    _xRangeMin = Some(v)
  }
  def xRangeMin = _xRangeMin getOrElse {
    getXValuesMin -.05 * getXValuesRangeSize
  }

  var _xRangeMax: Option[Double] = None
  def xRangeMax_=(v: Double): Unit = {
    _xRangeMax = Some(v)
  }
  def xRangeMax = _xRangeMax getOrElse {
    getXValuesMax +.05 * getXValuesRangeSize
  }
  ...
  def xGridInterval_=(v: Double): Unit = {
    _xGridInterval = Some(v)
  }
  def xGridInterval = _xGridInterval getOrElse {
    (xRangeMax - xRangeMin) / 8.01
  }
